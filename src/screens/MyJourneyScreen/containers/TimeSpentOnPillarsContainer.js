...
const merge = ({ modelsAndDataForExercise }) => {
  ...
  if (formData && notEmpty(formData)) {
    ...
    const typicalWeekTemplateObject = typicalWeek.reduce(
      (allPillars, pillar) => {
...
const TimeSpentOnPillarsContainer = compose(FormConsumer, mapProps(merge))(
