...
export function* _syncGoalsNotifications(action: {
  ...
  const careerNotifications = notifications.filter(
    n => n.type === notificationTypes.CAREER_GOAL_NOTIFICATION
  )
  ...
  const mergedGoalsData = goals.reduce((goalsData, goal) => {
    ...
    const notification = careerNotifications.find(n => {
      const { data } = n
      if (
        !data ||
  ...
  const notificationsActions = careerNotifications.reduce(
  ...
  for (const goalAction of actions) {
    ...
    if (_action === CREATE) {
      ...
      const additionalProps: { type: string, data: GoalNotificationPayload } = {
        type: notificationTypes.CAREER_GOAL_NOTIFICATION,
        data: {
          goalId: id,
          due,
          category,
...
function* watchForCareerGoalNotificationsSaga() {
  yield takeLatest(CAREER_GOAL_NOTIFICATION, _handleGoalNotification)
}

function* watchForCareerGoalSideEffect() {
  yield takeLatest(CAREER_GOALS_SIDE_EFFECT, _syncGoalsNotifications)
}
export function* watchForCareerGoalsSaga() {
  yield fork(watchForCareerGoalNotificationsSaga)
  yield fork(watchForCareerGoalSideEffect)
}
