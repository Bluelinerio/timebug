...
import { REQUEST, GET_ALL_STEPS_FROM_CMS, GET_STEP_FROM_CMS_BY_STEP } from '../constants/actionTypes'
import { incrementRequestCount, decrementRequestCount } from '../actions/network'
import { getAllStepsFromCMS, getStepFromCMSByStep, getStepsColorFromCMS } from '../actions/steps'
import { contentfulClient, CONTENTFUL_CONTENT_COLORS, CONTENTFUL_CONTENT_STEP } from '../clients/contentful'
...
function* getStepFromCMSByStepWorker(action: { number: number }) {
	...
	try {
		...
		if (yield cancelled()) yield put(decrementRequestCount())
	}
}
...
export function* getAllStepsSaga() {
	yield request(getAllStepsFromCMS, fetchSteps)
}
