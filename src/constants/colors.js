export const lightblue = '#48d0f1'
export const seafoamBlue = '#6AC2ED'
export const deepBlue = '#00098A'
export const lessSaturatedDeepBlue = '#21288A'
export const white = 'white'
export const white2 = '#f8f8f8'
export const white3 = '#fcfcfc'
export const lightGrey = '#fefefe'
export const white90 = 'rgba(255, 255, 255, 0.9)'
export const duskBlue = '#262d85'
export const hotPink = '#EB008B'
export const facebookColor = '#3b5998'
export const heavyGray = '#717171'
export const lightGray = '#AAAAAA'
export const mediumGray = '#E9E9E9'
export const gray400 = '#BDBDBD'
export const gray300 = '#E0E0E0'
export const paleBlue = '#E3F2FD'
export const lessPaleBlue = '#DEEDFD'
export const blue100 = '#B3E5FC'
export const gray200 = '#EEEEEE'
export const gray50 = '#FAFAFA'
export const gray900 = '#212121'
export const gray100 = '#F5F5F5'
export const gray600 = '#757575'
export const azure = '#4EADFF'
export const cyan = '#B8DEFF'
export const transparentCyan = 'rgba(184,222,255,0.5)'
export const darkBlue = '#005587'
export const blue900 = '#0D47A1'
export const greenA400 = '#00E676'
export const orange50 = '#FFF3E0'
export const green50 = '#E8F5E9'
export const red500 = '#F44336'
export const yellow500 = '#FFEB3B'
export const cyan500 = '#00BCD4'
export const black = '#000000'

export const MEDITATION = '#005587'
export const SELF_ASSESSMENT = '#F78C00'
export const VISION_CREATION = '#00B896'
export const COMPLETE = '#EB008B'

export const tabBarBackground = '#E0F2F1'
export const tabBarButtonColor = deepBlue
export const tabBarUnselected = 'gray'

/**
 * StartScreen
 */

export const StartScreenBannerColor = '#B8DEFF'

export const StartScreenButtonsColor = azure

export const StartScreenBackgroundColor = '#E1F3FB'

export const PHASE_1_COMPLETE = azure
export const PHASE_2_COMPLETE = '#F89A1F'
export const PHASE_3_COMPLETE = '#20B89D'
export const PHASE_1_INCOMPLETE = cyan
export const PHASE_2_INCOMPLETE = '#FAB152'
export const PHASE_3_INCOMPLETE = '#20DE9D'
export const PHASE_1_TEXT_AND_BUTTONS = darkBlue
export const PHASE_2_TEXT_AND_BUTTONS = '#D1821B'
export const PHASE_3_TEXT_AND_BUTTONS = '#199680'

export const whiteGradientColors = {
  startColor: white,
  endColor: white2,
}

/**
 * V2.0
 */

export const PHASE_1_BAR = azure
