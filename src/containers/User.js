...
type Props = {
  renderWithState:(state: UNDETERMINED | ANONYMOUS | AUTHENTICATING) => React.Node,
...
export default connect(mapStateToProps)(({
  ...
  if(typeof userState === 'string') { 
    ...
    if(userState === UNDETERMINED && renderWithUndetermined) return renderWithUndetermined()
    if(userState === ANONYMOUS && renderWithAnonymous) return renderWithAnonymous()
    if(userState === AUTHENTICATING && renderWithAuthenticating) return renderWithAuthenticating()
  }
  if(userState && typeof userState === 'object' && renderWithUser) return renderWithUser(userState)
  return null
})